...
from exceptions import InferenceTimeValueError
# create logger
...
def model_inference(model, dataloder_data, avg_sec=20):
    ...
    with torch.no_grad():
        ...
        if inference_time_ms > avg_sec:
            raise InferenceTimeValueError(f'Average inference time per image: {inference_time_ms:.4f}ms, '
                                          f'need to reduce {inference_time_ms-avg_sec}ms')
        else:
            logger.info(f'Average Inference Time per Image: {inference_time_ms:.4f}ms')
        logger.info(f'Inference Time: {elapsed_time:.6f} seconds')
...
def single_threaded_inference(model, input_data, avg_sec=20):
    ...
    try:
        model_inference(model, input_data)
    except InferenceTimeValueError as e:
        logger.error(f'Exceed allowed average inference time per image {avg_sec}')
...
if __name__ == '__main__':
    ...
    t_stop = process_time()
    logger.info(f'Elapsed time for inference only in seconds: {t_stop-t_inf_start}')
    logger.info(f'Elapsed time for entire inference in seconds: {t_stop-t_start}')
